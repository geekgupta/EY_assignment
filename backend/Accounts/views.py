...
cities_database = ['Mumbai', 'Pune', 'Dhaka', 'Delhi', 'New York', 'Los Angeles', 'Chicago', 'San Francisco', 'Miami', 'Houston', 'Seattle']

@csrf_exempt
@api_view(["POST"])
@permission_classes((AllowAny,))
def searchcity(request):  
    datax = request.data.get('city_name')
    matching_cities = []
    print('data', datax, request.data)
    if datax:
        datax = datax.lower()  # Convert the input to lowercase for case-insensitive matching
        for city in cities_database:
            if datax in city.lower():
                matching_cities.append(city)

    return Response({'matches': matching_cities})
